...
(function() {
	...
	ff.Button.prototype = {
		...
		press: function(isPressed) {
			this._app.netChannel.addMessageToQueue(true, ff.Constants.CMDS.BUTTON_PRESS, { id: this._id, value: isPressed });
		},
		
		click: function() {
			this._app.netChannel.addMessageToQueue(true, ff.Constants.CMDS.BUTTON_CLICK, { id: this._id });
		}
...
(function() {
	...
	ff.TactileZone = function(app, id) {
		this._app = app;
		this._id = id;
	};
	
	ff.TactileZone.prototype = {
...
//			div.ontouchcancel = that.touchHandler;
		...
		update: function() {
			this._app.netChannel.addMessageToQueue(false, ff.Constants.CMDS.TACTILE_MOVE,
					{ id: this._id, x: (this._mousePositionNormalized.x*100) << 0, y: (this._mousePositionNormalized.y*100) << 0 });
		},
		
		press: function(isPressed) {
			this._app.netChannel.addMessageToQueue(true, ff.Constants.CMDS.TACTILE_PRESS, { id: this._id, value: isPressed });
		}
	};
	...
	ff.ClientApp.prototype = {
		...
		registerElements: function() {
			...
			for (var i = 0; i < doc.length; i++){
				var button = new ff.Button(that, doc[i].id);
			...
			for (var i = 0; i < doc.length; i++){
				var div = new ff.TactileZone(that, doc[i].id);
